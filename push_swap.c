...
/*   Updated: 2024/03/05 17:22:57 by ohammou-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    random_numbers_3(t_list **stac_a)
{
        ...
        (*stac_a)->next->content > (*stac_a)->next->next->content)
    {
        ...
        (*stac_a)->next->content < (*stac_a)->next->next->content)
        ra(stac_a,1);
    else if((*stac_a)->content < (*stac_a)->next->content && (*stac_a)->content < (*stac_a)->next->next->content && 
        (*stac_a)->next->content >  (*stac_a)->next->next->content)
    {
        sa(stac_a,1);
        ra(stac_a,1);
...
int main(int ac,char **av)
{
    ...
    stac_b = NULL;
    stac_a = list(av);
    if(ft_lstsize(stac_a) == 2 || ac == 3)
    {
        if(stac_a->content > stac_a->next->content)
            sa(&stac_a,1);
        return 0;
    }
    if(ft_lstsize(stac_a) == 3)
